...
function runtimeWithKey(privateKey: Hex, rpcUrl: string): IAgentRuntime {
	...
	const backend: WalletBackendLike = {
		getAddresses: () => ({ evm: account.address, solana: null }),
		getEvmAccount: () => account,
		getSolanaSigner: () => {
			throw new Error("Solana signer is not configured for the EVM live test.");
		},
	};
	const walletService = {
...
describe.runIf(live)("Aomi live wallet execution", () => {
	it("changes chain state and surfaces a real insufficient-funds failure", async () => {
		...
		const result = await executeWalletRequest(
			runtimeWithKey(privateKey, rpcUrl),
			config,
			request,
		);
		...
		await expect(
			executeWalletRequest(
				runtimeWithKey(emptyKey, rpcUrl),
				config,
